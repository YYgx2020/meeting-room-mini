...
Page({
  ...
  */
  ...
  async bindrefresherrefresh1(e) {
    ...
    wx.$api.user.refresh(data).then(res => {
      console.log(res);
      ...
      if (current == 0) {
        ...
        approvalList = res.data.result.rows.reverse().map(item => {
          ...
          if (item.register_status === 1) {
            item.statusImg = '../../static/images/status/pass.png'
          } else if (item.register_status === 3) {
            item.statusImg = '../../static/images/status/no_pass.png'
          }
          return item;
        }) 
        ...
        if (approvalList.length !== 0) {
          approvalList = approvalList.filter(item => item.id !== wx.$userInfo.id);
        }
        if (firstIn2) {
          this.setData({
            approvalList,
  ...
  pass(e) {
    ...
    wx.$api.user.pass(data).then(res => {
      console.log(res);
      wx.showToast({
        title: '审核成功',
        mask: true,
